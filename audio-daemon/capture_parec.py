...
class AudioConfig:
    ...
    capture_sample_rate: int = 16000  # parec captures at target rate directly
    channels: int = 2
    dtype: str = "float32"
...
class AudioCapture:
    ...
    def _capture_worker(self):
        ...
        # --format = s16ne (signed 16-bit native endian PCM)
        # --latency = buffer latency in bytes
        ...
        try:
            ...
            # Each frame = channels * bytes_per_sample = 2 * 2 = 4 bytes
            frame_size = self.config.channels * self.sample_width  # 4 bytes for stereo 16-bit
            
            # Read 1024 frames at a time = 4096 bytes
            chunk_bytes = 1024 * frame_size
            
            while self.is_recording and self.parec_process.poll() is None:
                try:
                    raw_data = self.parec_process.stdout.read(chunk_bytes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="meetscribe_source", help="PulseAudio source name")
    ...
    print(f"Recording from device '{args.device}' for {args.duration} seconds (Ctrl+C to stop early)...")
    capture.start()
