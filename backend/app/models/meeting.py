...
from sqlalchemy import (
    ...
    ARRAY, JSON, Index, UniqueConstraint
)
...
class Meeting(Base):
    ...
    status = Column(String(20), nullable=False, default="idle")  # idle, recording, processing, completed, failed
    started_at = Column(DateTime(timezone=True), nullable=True)
    ended_at = Column(DateTime(timezone=True), nullable=True)
    duration_seconds = Column(Integer, nullable=True)
    audio_path = Column(String(1000), nullable=True)
    calendar_event_id = Column(UUID(as_uuid=True), ForeignKey("calendar_events.id"), nullable=True)
...
class CalendarConnection(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_calendar_conn_user_provider', 'user_id', 'provider'),
    )
...
class CalendarEvent(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    connection_id = Column(UUID(as_uuid=True), ForeignKey("calendar_connections.id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        ...
        UniqueConstraint('connection_id', 'provider_event_id', name='uix_calendar_event'),
    )
...
class JobStatus(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    meeting_id = Column(UUID(as_uuid=True), ForeignKey("meetings.id", ondelete="CASCADE"), nullable=False)
    job_type = Column(String(50), nullable=False)  # transcription, summarization, export, etc.
