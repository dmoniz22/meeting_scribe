...
async def get_or_create_default_user(db: AsyncSession):
    ...
    if not user:
        user = User(email="dev@meetscribe.local", display_name="Developer")
        db.add(user)
        await db.commit()
...
async def create_meeting(
    meeting_data: MeetingCreate, db: AsyncSession = Depends(get_db)
):
    """Create a new meeting record."""
    user = await get_or_create_default_user(db)

    meeting = Meeting(user_id=user.id, title=meeting_data.title, status="idle")
    db.add(meeting)
    await db.commit()
...
async def update_meeting(
    meeting_id: UUID, meeting_data: MeetingUpdate, db: AsyncSession = Depends(get_db)
):
