...
RUN echo "CREATE EXTENSION IF NOT EXISTS pg_trgm;" > /docker-entrypoint-initdb.d/01-enable-extensions.sql && \
    echo "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";" >> /docker-entrypoint-initdb.d/01-enable-extensions.sql

# Note: pgvector extension will be created by the application or via a migration
# since it needs to be created in the specific database
